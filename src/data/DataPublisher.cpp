...
void DataPublisher::send_group(const ChannelFramePtr& frame) {
    ...
    switch (frame->group) {
        case DataGroup::Visual2D:   sock = sock_2d_;     topic = "visual_geometric_2d";   break;
        case DataGroup::Visual3D:   sock = sock_3d_;     topic = "visual_geometric_3d";   break;
        case DataGroup::SensorData: sock = sock_sensor_; topic = "sensor_tracking";       break;
    }
    if (!sock) return;
    send_multipart(sock, topic, frame);
}

bool DataPublisher::send_multipart(void* sock, const std::string& topic,
    ...
    for (const auto& b : frame->bundles) {
        nlohmann::json p;
        p["id"] = data_type_to_string(b->type);
        p["size"] = b->data.size();
        p["ts_sec"] = b->ts_sec;
        p["ts_nsec"] = b->ts_nsec;
    ...
    int rc = send_frame(sock, topic.data(), topic.size(), dontwait | ZMQ_SNDMORE, nullptr);
    if (rc < 0) {
        dropped_hwm_.fetch_add(1, std::memory_order_relaxed);
        return false;
    }
    rc = send_frame(sock, header_str.data(), header_str.size(), dontwait | ZMQ_SNDMORE, nullptr);
    if (rc < 0) {
        dropped_hwm_.fetch_add(1, std::memory_order_relaxed);
        return false;
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (rc < 0) {
            ...
            SIM_LOG(LogLevel::WARN, "DataPublisher",
                    "HWM drop on " + topic + " frame=" + std::to_string(frame->frame_index));
            return false;
        }
    }
...
void DataPublisher::publish_shutdown() {
    auto send_ctrl = [this](void* sock, const char* topic) {
        if (!sock) return;
        nlohmann::json h;
        ...
        int rc = send_frame(sock, topic, std::strlen(topic), ZMQ_DONTWAIT | ZMQ_SNDMORE, nullptr);
        if (rc >= 0) {
            send_frame(sock, s.data(), s.size(), ZMQ_DONTWAIT, nullptr);
            shutdown_sent_.fetch_add(1, std::memory_order_relaxed);
        }
    };
    send_ctrl(sock_2d_, "visual_geometric_2d");
    send_ctrl(sock_3d_, "visual_geometric_3d");
    send_ctrl(sock_sensor_, "sensor_tracking");
